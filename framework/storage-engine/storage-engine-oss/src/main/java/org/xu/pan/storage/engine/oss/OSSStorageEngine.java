...
public class OSSStorageEngine extends AbstractStorageEngine {
    ...
    protected void doStoreChunk(StoreFileChunkContext context) throws IOException {
        ...
        if (Objects.isNull(entity)) {
            entity = initChunkUpload(context.getFilename(), cacheKey); // 这里为什么不用suffix
        }
        ...
        if (Objects.isNull(result)) {
            throw new YPanFrameworkException("文件分片上传失败");
        }
    ...
    protected void doMergeFile(MergeFileContext context) throws IOException {
        ...
        if (Objects.isNull(entity)) {
            throw new YPanFrameworkException("文件分片合并失败，文件的唯一标识为：" + context.getIdentifier());
        }
    ...
    protected void doReadFile(ReadFileContext context) throws IOException {
        ...
        if (Objects.isNull(ossObject)) {
            throw new YPanFrameworkException("文件读取失败，文件的名称为：" + context.getRealPath());
        }
    ...
    private String getBaseUrl(String url) {
        if (StringUtils.isBlank(url)) {
            return YPanConstants.EMPTY_STR;
        }
        if (checkHaveParams(url)) {
            return url.split(getSplitMark(YPanConstants.QUESTION_MARK_STR))[0];
        }
        return url;
    }
    ...
    private JSONObject analysisUrlParams(String url) {
        ...
        String paramsPart = url.split(getSplitMark(YPanConstants.QUESTION_MARK_STR))[1];
        if (StringUtils.isNotBlank(paramsPart)) {
            List<String> paramPairList = Splitter.on(YPanConstants.AND_MARK_STR).splitToList(paramsPart);
            paramPairList.stream().forEach(paramPair -> {
                String[] paramArr = paramPair.split(getSplitMark(YPanConstants.EQUALS_MARK_STR));
                if (paramArr != null && paramArr.length == YPanConstants.TWO_INT) {
    ...
    private boolean checkHaveParams(String url) {
        return StringUtils.isNotBlank(url) && url.indexOf(YPanConstants.QUESTION_MARK_STR) != YPanConstants.MINUS_ONE_INT;
    }
    ...
    private String getSplitMark(String mark) {
        return new StringBuffer(YPanConstants.LEFT_BRACKET_STR)
          .append(mark)
          .append(YPanConstants.RIGHT_BRACKET_STR)
          .toString();
    }
    ...
    private String assembleUrl(String baseUrl, JSONObject params) {
        ...
        urlStringBuffer.append(YPanConstants.QUESTION_MARK_STR);
        List<String> paramsList = Lists.newArrayList();
        StringBuffer urlParamsStringBuffer = new StringBuffer();
        params.entrySet().forEach(entry -> {
            urlParamsStringBuffer.setLength(YPanConstants.ZERO_INT);
            urlParamsStringBuffer.append(entry.getKey());
            urlParamsStringBuffer.append(YPanConstants.EQUALS_MARK_STR);
            urlParamsStringBuffer.append(entry.getValue());
            paramsList.add(urlParamsStringBuffer.toString());
        });
        return urlStringBuffer.append(Joiner.on(YPanConstants.AND_MARK_STR).join(paramsList)).toString();
    }
    ...
    private ChunkUploadEntity initChunkUpload(String filename, String cacheKey) {
        ...
        if (Objects.isNull(result)) {
            throw new YPanFrameworkException("文件分片上传初始化失败");
        }
    ...
    private String getFilePath(String fileSuffix) {
        return new StringBuffer()
          ...
          .append(YPanConstants.SLASH_STR)
