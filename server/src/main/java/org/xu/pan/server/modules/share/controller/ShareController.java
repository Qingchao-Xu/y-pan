package org.xu.pan.server.modules.share.controller;
...
import org.xu.pan.server.common.annotation.NeedShareCode;
import org.xu.pan.server.common.utils.ShareIdUtil;
...
import org.xu.pan.server.modules.share.converter.ShareConverter;
import org.xu.pan.server.modules.share.po.CancelSharePO;
import org.xu.pan.server.modules.share.po.CheckShareCodePO;
import org.xu.pan.server.modules.share.po.CreateShareUrlPO;
import org.xu.pan.server.modules.share.po.ShareSavePO;
...
public class ShareController {
    ...
    @PostMapping("share")
    public R<YPanShareUrlVO> create(@Validated @RequestBody CreateShareUrlPO createShareUrlPO) {
        ...
        List<Long> shareFileIdList = Splitter.on(YPanConstants.COMMON_SEPARATOR).splitToList(shareFileIds).stream().map(IdUtil::decrypt).collect(Collectors.toList());

        context.setShareFileIdList(shareFileIdList);

        YPanShareUrlVO vo = iShareService.create(context);
        return R.data(vo);
    }
    ...
    @GetMapping("shares")
    public R<List<YPanShareUrlListVO>> getShares() {
        QueryShareListContext context = new QueryShareListContext();
        context.setUserId(UserIdUtil.get());
        List<YPanShareUrlListVO> result = iShareService.getShares(context);
    ...
    public R cancelShare(@Validated @RequestBody CancelSharePO cancelSharePO) {
        ...
        List<Long> shareIdList = Splitter.on(YPanConstants.COMMON_SEPARATOR).splitToList(shareIds).stream().map(IdUtil::decrypt).collect(Collectors.toList());
    ...
    @NeedShareCode
    @LoginIgnore
    public R<List<YPanUserFileVO>> fileList(@NotBlank(message = "文件的父ID不能为空") @RequestParam(value = "parentId", required = false) String parentId) {
        QueryChildFileListContext context = new QueryChildFileListContext();
        context.setShareId(ShareIdUtil.get());
        context.setParentId(IdUtil.decrypt(parentId));
        List<YPanUserFileVO> result = iShareService.fileList(context);
        return R.data(result);
    }
